...
| `cbz_sanitizer.py` | Always | **Yes** | Batch sanitizer — walks a library folder and applies the full cleaning/tagging pipeline in-place; supports `--sort`, `--resume`, `--restart`, `--dry-run`, and `--workers` |
...
```powershell
# All run recursively by default; all support --workers N
...
python scripts\cbz_deduplicator.py --workers 8
python scripts\cbz_sanitizer.py --workers 1          # serial (original behaviour)
...
All batch tools (except `cbz_watcher.py` and `cbz_number_tagger.py`) now support parallel processing via `--workers N`:
...
| [docs/cbz_sanitizer.md](docs/cbz_sanitizer.md) | Full CLI reference, sort modes, progress/resume system, parallel processing |
