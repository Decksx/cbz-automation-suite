...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run bounded, read-only exact page-content hashing for CBZ "
            "archives."
        )
    )
    ...
    *,
    ...
    with database_connection(database) as connection:
        ...
        worker = JobWorker(
            queue,
            {
                "hash_archive_pages": HashArchivePagesHandler(
                    connection
                )
            },
            worker_id=f"{socket.gethostname()}:bounded-page-hash-cli",
        ...
        page_count = connection.execute(
            "SELECT COUNT(*) FROM page_hashes"
        ).fetchone()[0]
        duplicates = repository.duplicate_content_groups()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Page-content hashing failed: {exc}", file=sys.stderr)
        return 1

    print("Bounded page-content hashing completed.")
    ...
    print(f"Pages hashed:      {output['pages_hashed']}")
