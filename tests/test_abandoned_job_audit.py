...
def test_wal_commit_can_leave_the_file_fingerprint_unchanged(
    ...
    try:
        ...
        seed_job(writer, status="claimed", claimed_at=STALE)
...
def test_external_wal_commit_mid_audit_invalidates_the_report(
    ...
    The commit is injected inside `collect_stale_jobs`, i.e. squarely
    inside the audit's read window -- exactly where the old
    fingerprint-only guard was blind.
    """
    database = tmp_path / "audit.db"
    ...
    import comic_automation.jobs.abandoned_job_audit as audit_module

    real_collect = audit_module.collect_stale_jobs
    fingerprint_at_commit: dict[str, object] = {}

    def collect_then_external_commit(connection, **kwargs):
        result = real_collect(connection, **kwargs)
        ...
        with database_connection(database) as other:
            seed_job(other, status="running", claimed_at=VERY_STALE)
    ...
    monkeypatch.setattr(
        audit_module, "collect_stale_jobs", collect_then_external_commit
    )
...
def test_report_surfaces_the_snapshot_boundary(tmp_path: Path) -> None:
    ...
    # The legacy key keeps its value but is now labelled as diagnostic.
    assert output["database_unchanged"] is True
