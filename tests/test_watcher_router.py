...
def test_the_summary_is_safe_with_nothing_classified(tmp_path):
    assert "no directories" in _router(tmp_path).summarise()
...
def test_a_completed_move_makes_the_series_sticky(tmp_path):
    ...
    router.note_move("New Series", "manga", tmp_path / "Manga" / "New Series")
    hit = router.index.lookup("New Series")
    assert hit == ("manga", tmp_path / "Manga" / "New Series")
...
def test_a_move_to_an_unknown_destination_is_not_indexed(tmp_path):
    router = _router(tmp_path)
    router.note_move("Whatever", "nonexistent", tmp_path / "x")
