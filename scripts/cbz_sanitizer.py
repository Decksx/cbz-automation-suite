...
UNKNOWN_CHAPTER_RE = re.compile(
    r'^unknown[\s_]chapter[\s_]*(\d[\d.]*)(.*?)$', re.IGNORECASE
)
...
def detect_and_fix_compilations(
    ...
    for suspect, start, end in candidates:
        ...
        log.info(f"    Compilation detected: '{cbz.name}' looks like ch.{_fmt_num(start)}-{_fmt_num(end)}")
        if dry_run:
...
def _process_series_dir(
    ...
    This function runs in a worker thread — it must not mutate shared state except
    via thread-safe mechanisms (logging, save_progress).
    """
    ...
    return total_processed, total_renamed, total_skipped, total_comp_fixed
...
def sanitize_directory(dir_path: Path, processed: set, started: str, workers: int) -> None:
    ...
    if workers == 1:
        ...
        # Parallel path — each series directory is an independent task
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(future_to_dir):
                comic_dir = future_to_dir[future]
                try:
                    p, r, s, c = future.result()
                    total_processed += p; total_renamed += r
                    total_skipped += s;   total_comp_fixed += c
                except Exception as e:
                    log.error(f"  Worker failed for '{comic_dir.name}': {e}")
    ...
    for arg in args_raw:
        if arg.startswith("--workers="):
            try:
                ...
                pass
        elif arg == "--workers" and args_raw.index(arg) + 1 < len(args_raw):
            try:
                workers = max(1, int(args_raw[args_raw.index(arg) + 1]))
            except ValueError:
                pass
