...
# ─────────────────────────────────────────────
SCAN_FOLDER    = r"\\tower\media\comics\manga"   # Folder to sanitize
...
def sanitize(text: str) -> str:
    ...
    and XML fields. Steps in order:
      1. Decode XML/HTML entities
      2. Remove website URLs
      3. Remove scanlator group names
      4. Strip trailing slash(es) and G-code suffix
      5. Remove non-Latin/non-Greek/non-emoji characters
      6. Remove [bracketed] and (parenthesised) groups
      7. Strip stray bracket/parenthesis characters
...
def process_comicinfo(
    ...
    - Found:   check/fix Title, Series, Number, and Volume tags.
    - Missing: inject a fresh comicinfo.xml from template.

    prefetched_xml: optional (entry_name, xml_text) already read by the caller.
    """
...
def _merge_directories(src_dir: Path, dest_dir: Path) -> None:
    """Merge src_dir into dest_dir recursively. Larger file wins on conflict."""
...
def process_cbz_file(cbz_path: Path, override_name: str | None = None) -> Path:
    ...
    if new_name != cbz_path.name:
        ...
        if new_path.exists():
            ...
            if src_size > dest_size:
                log.info(f"    Rename collision: incoming ({src_size:,} B) > existing ({dest_size:,} B) - replacing.")
                cbz_path.replace(new_path)
                cbz_path = new_path
            else:
                log.info(f"    Rename collision: existing ({dest_size:,} B) >= incoming ({src_size:,} B) - discarding incoming.")
                cbz_path.unlink()
                return new_path
        else:
...
def sanitize_directory(dir_path: Path, processed: set, started: str) -> None:
    """Recursively find and process all .cbz files under dir_path."""
    log.info("=" * 60)
    ...
    total_processed = total_skipped = total_renamed = total_comp_fixed = 0
...
def main():
    ...
    if sort_mode == "newest":
        ...
        subdirs = sorted((d for d in scan_path.iterdir() if d.is_dir()), key=lambda d: d.name.lower())
    else:
        subdirs = sorted((d for d in scan_path.iterdir() if d.is_dir()), key=lambda d: d.name.lower(), reverse=True)
